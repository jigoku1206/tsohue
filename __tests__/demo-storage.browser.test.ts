...
describe('loadState', () => {
  ...
  it('backfills missing recurring_rules field', () => {
    const state = buildSeedState()
    const withoutField = { ...state, recurring_rules: undefined }
    delete withoutField.recurring_rules
  ...
  it('backfills missing ledger_budgets field', () => {
    const state = buildSeedState()
    const withoutField = { ...state, ledger_budgets: undefined }
    delete withoutField.ledger_budgets
    localStorage.setItem('demo_state_v1', JSON.stringify(withoutField))
    const loaded = loadState()
